...
class MapContainer extends React.Component {
    ...
    componentDidMount () {
        ...
        hutsData.forEach(item => {
            L.marker([item.geometry.coordinates[1], item.geometry.coordinates[0]], {icon: greenIcon}).addTo(this.map)
            .bindPopup(`<p><b>${item.properties.Name}</b></p>${item.properties.description}`, customOptions);
          })
    ...
    render() {
        ...
        return <div className="ui fluid container" id="map" style={{minHeight: "92vh", minWidth: "100%", margin:0, position: "absolute" }} ></div>;
